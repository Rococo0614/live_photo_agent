...
def test_execute_end_to_end(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(settings, "local_model_dir", tmp_path / "fake-model")
    ...
    def fake_call_local_hf_planner(self, request: AgentRequest, library_summary: dict[str, object]) -> ExecutionPlan:
    ...
    monkeypatch.setattr(QwenPlanner, "_call_local_hf_planner", fake_call_local_hf_planner)
